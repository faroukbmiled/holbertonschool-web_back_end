...
def profile_unlogged() -> None:
    """Test accessing profile with a non-existent session_id."""
    r = requests.get('http://127.0.0.1:5000/profile')
    assert r.status_code == 403


def profile_logged(session_id: str) -> None:
    """Test accessing profile with a valid session_id."""
...
def reset_password_token(email: str) -> str:
    """Check that a password reset token gets generated."""
...
def update_password(email: str, reset_token: str, new_password: str) -> None:
    """Change the password using a reset token."""
